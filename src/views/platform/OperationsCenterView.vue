...
import { getReviewBatch, reviewRecords } from './manual-review-data'
...
const pageMeta = computed(() => ({
  alerts: { title: '异常治理', subtitle: '发现、阻断和处置数据处理、图谱构建及服务运行异常' },
  review: { title: '人工处理', subtitle: '与任务中心共用数据更新批次和处理实例 ID，逐条处理已隔离的实体、关系和属性异常' },
})[props.mode])

const alertRows = [
  { id: 'ALT-0714-018', level: '严重', category: '大模型抽取', module: '图谱构建', node: '大模型抽取', domain: '论文', batch: 'UPD-20260714', reason: '输出未通过 Schema 与来源证据交叉校验', impact: '326 条隔离 · 下游阻断', owner: '张建图', time: '07-14 10:24', status: '待处理', blocked: true, target: '/tasks?module=图谱构建&batch=UPD-20260714' },
  { id: 'ALT-0714-014', level: '严重', category: '数据质量', module: '数据处理', node: '质量检验', domain: '论文', batch: 'UPD-20260714', reason: '唯一性、必填和枚举规则校验失败', impact: '385 条隔离 · 下游阻断', owner: '李质量', time: '07-14 09:48', status: '待处理', blocked: true, target: '/tasks?module=数据处理&batch=UPD-20260714' },
  { id: 'ALT-0713-012', level: '警告', category: '服务运行', module: '服务调用', node: '专家关系 Tool', domain: '人才', batch: 'CALL-20260713-1058', reason: 'P95 响应耗时超过 2 秒阈值', impact: '18 次慢调用 · 未阻断', owner: '李运维', time: '07-13 10:58', status: '处理中', blocked: false, target: '/graph-tools' },
  { id: 'ALT-0713-106', level: '警告', category: '实体对齐', module: '图谱构建', node: '实体消歧', domain: '专利', batch: 'UPD-20260713', reason: '候选实体置信度低于 0.75', impact: '42 个候选 · 已处理', owner: '王审核', time: '07-13 18:06', status: '已关闭', blocked: false, target: '/tasks?module=图谱构建&batch=UPD-20260713' },
  { id: 'ALT-0712-099', level: '提示', category: 'Schema 校验', module: '图谱构建', node: '字段映射', domain: '企业', batch: 'KG-INC-20260712-099', reason: '2 个新增字段未匹配当前 Schema', impact: '12 条隔离 · 已恢复', owner: '张建图', time: '07-12 14:08', status: '已关闭', blocked: false, target: '/schema' },
  { id: 'ALT-0712-088', level: '提示', category: '服务运行', module: '服务调用', node: '子图分析 Tool', domain: '产业链', batch: 'CALL-20260712-1031', reason: '查询结果达到最大节点数量限制', impact: '3 次截断 · 未阻断', owner: '王算法', time: '07-12 10:31', status: '已关闭', blocked: false, target: '/graph-tools' },
  { id: 'ALT-0711-072', level: '警告', category: '图谱入库', module: '图谱构建', node: '增量写入', domain: '人才', batch: 'KG-FULL-20260711-008', reason: '写入吞吐低于近七日基线 20%', impact: '延迟 26 分钟 · 已恢复', owner: '张建图', time: '07-11 23:46', status: '已关闭', blocked: false, target: '/task-detail/construction/KG-FULL-20260711-008?step=persist' },
  { id: 'ALT-0711-061', level: '提示', category: '数据质量', module: '数据处理', node: '标准表写入', domain: '专利', batch: 'DP-20260711-1800', reason: '任务执行时间接近超时阈值', impact: '1 个批次 · 已恢复', owner: '李质量', time: '07-11 18:22', status: '已关闭', blocked: false, target: '/task-detail/processing/DP-20260711-1800?step=write' },
]


const metrics = computed(() => ({
  alerts: [
    { label: '严重待处理', value: '2', hint: '需要立即确认' },
    { label: '已阻断任务', value: '2', hint: '错误未继续下游' },
  ...
  review: [
    { label: '待处理实例', value: '668', hint: '归属 1 个数据更新批次' },
    { label: '处理中实例', value: '43', hint: '使用统一处理实例 ID' },
    { label: '今日已处理', value: '42', hint: '审核后仍可查看详情' },
  ],
})[props.mode])

const primaryActionLabel = computed(() => ({ alerts: '刷新检测结果', review: '' })[props.mode])
...
const filteredAlertRows = computed(() => alertRows.filter((row) => {
    ...
    return (!keyword.value || text.includes(keyword.value))
      ...
      && (severity.value === '全部级别' || text.includes(severity.value))
...
const reviewRows = computed(() => reviewRecords.map((row) => {
  ...
  return { ...row, dataWindow: batch?.dataWindow ?? '-', severity: batch?.severity ?? '提示' }
}))
...
<template>
  <div class="ops-page">
    <header class="ops-head">
      ...
      <button v-if="mode === 'alerts'" class="primary" type="button" @click="runPrimaryAction">{{ primaryActionLabel }}</button>
    </header>

    <section :class="['ops-metrics', { 'is-alerts': mode === 'alerts', 'is-review': mode === 'review' }]">
      <article v-for="item in metrics" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}</strong><em>{{ item.hint }}</em></article>
    </section>
    ...
    <section class="ops-panel">
      ...
      <div :class="['ops-filter', { 'is-review': mode === 'review' }]">
        <input v-model="keyword" :placeholder="mode === 'review' ? '搜索处理实例 ID、对象、来源记录或异常原因' : '搜索批次、对象、异常原因'" />
        <select v-if="mode === 'alerts'" v-model="severity"><option>全部级别</option><option>严重</option><option>警告</option><option>提示</option></select>
        ...
        <select v-model="status"><option>全部状态</option><option v-if="mode === 'alerts' || reviewTab === '待处理'">待处理</option><option v-if="mode === 'alerts' || reviewTab === '待处理'">处理中</option><option v-if="mode === 'review' && reviewTab === '历史记录'">已完成</option><option v-if="mode === 'alerts'">已关闭</option></select>
        <button type="button" @click="resetFilters">清空筛选</button>
      </div>

      <div v-if="mode === 'alerts'" class="ops-table-scroll"><table>
        <thead><tr><th>异常编号 / 级别</th><th>异常类型</th><th>模块 / 节点</th><th>业务域</th><th>任务批次</th><th>异常说明</th><th>影响范围</th><th>责任人</th><th>发生时间</th><th>状态</th><th>操作</th></tr></thead>
        <tbody><tr v-for="row in filteredAlertRows" :key="row.id"><td><strong>{{ row.id }}</strong><small :class="`level-${row.level}`">{{ row.level }}</small></td><td><b>{{ row.category }}</b></td><td><b>{{ row.module }}</b><small>{{ row.node }}</small></td><td>{{ row.domain }}</td><td><code>{{ row.batch }}</code></td><td class="alert-reason">{{ row.reason }}</td><td>{{ row.impact }}</td><td>{{ row.owner }}</td><td>{{ row.time }}</td><td><span :class="['alert-status', `is-${row.status}`]">{{ row.status }}</span></td><td><div class="alert-actions"><RouterLink :to="row.target">查看诊断</RouterLink><RouterLink v-if="row.status !== '已关闭' && row.module !== '服务调用'" :to="`/manual-review?batch=${row.batch}`">人工审核</RouterLink></div></td></tr></tbody>
      </table></div>

      <div v-else class="ops-review-table-scroll"><table>
        <thead><tr><th>处理实例 ID</th><th>待处理对象</th><th>处理类型 / 数据域</th><th>异常类型与原因</th><th>来源记录</th><th>所属更新批次</th><th>数据更新时间范围</th><th>置信度 / 风险</th><th>处理人</th><th>状态</th><th>{{ reviewTab === '历史记录' ? '完成时间' : '提交时间' }}</th><th>操作</th></tr></thead>
        <tbody><tr v-for="row in filteredReviewRows" :key="row.id"><td><code>{{ row.id }}</code></td><td><strong>{{ row.object }}</strong><small>{{ row.objectType }} · {{ row.objectId }}</small></td><td><b>{{ row.module }}</b><small>{{ row.domain }}域 · {{ row.node }}</small></td><td class="review-evidence"><b>{{ row.type }}</b><small>{{ row.evidence }}</small></td><td>{{ row.sourceTable }}<small>{{ row.sourceRecordId }}</small></td><td><code>{{ row.batch }}</code></td><td>{{ row.dataWindow }}</td><td><b>{{ row.score }}</b><small :class="`level-${row.severity}`">{{ row.severity }}</small></td><td>{{ row.handler }}</td><td><span :class="['review-status', `is-${row.status}`]">{{ row.status }}</span></td><td>{{ row.completedAt || row.updatedAt }}</td><td><RouterLink class="link" :to="`/manual-review/task/${row.id}`">{{ row.status === '已完成' ? '查看记录' : row.status === '处理中' ? '查看进度' : '进入处理' }} →</RouterLink></td></tr><tr v-if="!filteredReviewRows.length"><td class="review-empty" colspan="12">暂无符合条件的{{ reviewTab }}</td></tr></tbody>
      </table></div>
      ...
      <footer v-else class="review-pagination"><span>当前展示 {{ filteredReviewRows.length }} 条示例，实际系统按服务端分页加载</span><nav><button type="button" disabled>上一页</button><button class="active" type="button">1</button><button type="button">下一页</button></nav></footer>
    </section>
