...
import { getReviewBatch, getReviewConfidence, getReviewPriority, reviewRecords } from './manual-review-data'
...
const pageMeta = computed(() => ({
  alerts: { title: '异常治理', subtitle: '数据处理、图谱构建与服务运行' },
  review: { title: '人工处理', subtitle: '待处理实体、关系、属性与流程异常' },
})[props.mode])
...
const metrics = computed(() => ({
  ...
  review: [
    { label: '待处理实例', value: '711', hint: '提交前均保留在人工处理队列' },
    { label: '已完成实例', value: '42', hint: '提交后重跑进度转至任务中心' },
  ],
})[props.mode])
...
const reviewRows = computed(() => reviewRecords.map((row) => {
  ...
  const priority = getReviewPriority(row)
  return {
    ...row,
  ...
  <div class="ops-page">
    ...
    <section class="ops-panel">
      ...
      <div v-else class="ops-review-table-scroll"><table>
        <thead><tr><th>处理实例 ID</th><th>待处理对象</th><th>处理类型 / 数据域</th><th>来源记录</th><th>所属更新批次</th><th>数据更新时间范围</th><th>结果置信度</th><th>处置优先级</th><th>处理人</th><th>状态</th><th>{{ reviewTab === '历史记录' ? '完成时间' : '提交时间' }}</th><th>操作</th></tr></thead>
        <tbody><tr v-for="row in filteredReviewRows" :key="row.id"><td><code>{{ row.id }}</code></td><td><strong>{{ row.object }}</strong><small>{{ row.objectType }} · {{ row.objectId }}</small></td><td><b>{{ row.module }}</b><small>{{ row.domain }}域 · {{ row.node }}</small></td><td>{{ row.sourceTable }}<small>{{ row.sourceRecordId }}</small></td><td><code>{{ row.batch }}</code></td><td>{{ row.dataWindow }}</td><td class="review-confidence-cell"><b>{{ row.confidenceValue }}</b><em v-if="row.confidenceLabel">{{ row.confidenceLabel }}</em></td><td class="review-priority-cell"><em :class="`priority-${row.priority}`">{{ row.priorityScope }}</em><span>{{ row.priority === 'P0' ? '影响公共流程，已阻断' : '仅影响当前任务' }}</span></td><td>{{ row.handler }}</td><td><span :class="['review-status', `is-${row.status}`]">{{ row.status }}</span></td><td>{{ row.completedAt || row.updatedAt }}</td><td><RouterLink class="link" :to="`/manual-review/task/${row.id}`">{{ row.status === '已完成' ? '查看记录' : '进入处理' }} →</RouterLink></td></tr><tr v-if="!filteredReviewRows.length"><td class="review-empty" colspan="12">暂无符合条件的{{ reviewTab }}</td></tr></tbody>
      </table></div>
